...
# }
# 
# initialize.daily.sd <- function(values)
# {
...
#     curr_day_last_row_ind <- curr_day_first_row_ind+ROWS_PER_DAY-1
#     
#     compute.sd.per.row <- function(row_ind) { return (sd(values[curr_day_first_row_ind:row_ind])) }
#     return (sapply(curr_day_first_row_ind:curr_day_last_row_ind, compute.sd.per.row))
#   }
#   
#   return (c(sapply(1:ndays(values), init.daily.sd.for.single.day)))
# }
